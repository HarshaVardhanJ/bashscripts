...
if [ "$userOption" == "a" ]; then
     ...
     echo "Give the format, name and size (%, G, M, tb, R) for each partition in that order (everything separated by a space)."
     ...
     sleep 3
     diskutil partitionDisk "$DISK" 2 "$partitionScheme" "$partition1F" "$partition1N" "$partition1S" "$partition2F" "$partition2N" "$partition2S"
